...
```
使用 delegate_to_reasonix，读取 {路径/文件名}，帮我总结它的功能。
```

### 1.2 读文件指定范围

```
使用 delegate_to_reasonix，读取 {路径/文件名} 的第 {30} 到 {80} 行。
```

### 1.3 同时读多个文件做对比

```
使用 delegate_to_reasonix，执行以下任务：
1. 读取 {src/file-a.ts}
2. 读取 {src/file-b.ts}
...
```
使用 delegate_to_reasonix，搜索项目中所有出现 {关键字} 的地方。
```

### 2.2 限定目录搜索

```
使用 delegate_to_reasonix，搜索 {src/utils} 目录下所有出现 {关键字} 的地方。
```
...
```
使用 delegate_to_reasonix，运行 git status 和 git diff --name-only。
```

### 3.2 查看最近提交

```
使用 delegate_to_reasonix，查看最近 {5} 条 git 提交记录（git log --oneline -5）。
```

### 3.3 查看某个文件的修改历史

```
使用 delegate_to_reasonix，运行 git log --oneline -- {路径/文件名}，并告诉我每次提交改了些什么。
```
...
```
使用 delegate_to_reasonix，对 {src/} 目录下所有 .ts 文件运行 node --check。
```

### 5.2 运行测试

```
使用 delegate_to_reasonix，运行 npm test 并报告结果（通过的用例数和失败的用例数）。
```
...
```
第 1 步：使用 delegate_to_reasonix，搜索 {旧函数名} 的所有调用位置。
```

```
第 2 步：使用 delegate_to_reasonix，将 {src/xxx.ts} 中的 {旧函数} 重构为 {新函数}。
```

```
第 3 步：使用 delegate_to_reasonix，运行 node --check 验证语法。
```
...
3. **小步提交**：每个 `delegate_to_reasonix` 只做一个任务，失败了不影响其他部分
